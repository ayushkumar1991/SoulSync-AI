!function() {
  ...
  function movePlainYearMonth(getCalendarOps, doSubtract, plainYearMonthSlots, durationSlots, options = Object.create(null)) {
  ...
  function createCalendarFieldMethods(methodNameMap, alsoAccept) {
    ...
    for (const methodName in methodNameMap) {
      methods[methodName] = ({native: native}, dateArg) => {
        const argSlots = getSlots(dateArg) || {}, {branding: branding} = argSlots, refinedSlots = branding === PlainDateBranding || alsoAccept.includes(branding) ? argSlots : toPlainDateSlots(dateArg);
  ...
  function createCalendarGetters(methodNameMap) {
    ...
    for (const methodName in methodNameMap) {
      methods[methodName] = slots => {
        ...
        return (calendarSlot = calendar, "string" == typeof calendarSlot ? createNativeStandardOps(calendarSlot) : (calendarProtocol = calendarSlot, 
  ...
  function toPlainMonthDaySlots(arg, options) {
    if (options = copyOptions(options), isObjectLike(arg)) {
      ...
      return refinePlainMonthDayBag(createMonthDayRefineOps(calendarMaybe || isoCalendarId), !calendarMaybe, arg, options);
    }
  ...
  function getOffsetNanosecondsForAdapter(timeZoneProtocol, getOffsetNanosecondsFor, epochNano) {
    return validateTimeZoneOffset(requireInteger(getOffsetNanosecondsFor.call(timeZoneProtocol, createInstant(createInstantSlots(epochNano)))));
  }
  ...
  function refineTimeZoneSlot(arg) {
    return isObjectLike(arg) ? (getSlots(arg) || {}).timeZone || validateTimeZoneProtocol(arg) : (arg => resolveTimeZoneId((s => {
  ...
  function createTimeZoneOps(timeZoneSlot, adapterFuncs) {
    return "string" == typeof timeZoneSlot ? queryNativeTimeZone(timeZoneSlot) : createAdapterOps(timeZoneSlot, adapterFuncs);
  }
  ...
  function toPlainYearMonthSlots(arg, options) {
    if (options = copyOptions(options), isObjectLike(arg)) {
      ...
      slots) : refinePlainYearMonthBag(createYearMonthRefineOps(getCalendarSlotFromBag(arg)), arg, options);
    }
    const res = ((getCalendarOps, s) => {
  ...
  function toZonedDateTimeSlots(arg, options) {
    if (options = copyOptions(options), isObjectLike(arg)) {
      ...
      return ((refineTimeZoneArg, getTimeZoneOps, calendarOps, calendarSlot, bag, options) => {
      ...
      })(refineTimeZoneSlot, createTimeZoneOps, createDateRefineOps(calendarSlot), calendarSlot, arg, options);
    }
    return ((s, options) => {
      const organized = parseDateTimeLike(requireString(s));
  ...
  function toPlainTimeSlots(arg, options) {
    if (isObjectLike(arg)) {
      ...
      switch (slots.branding) {
       ...
       case ZonedDateTimeBranding:
        return refineOverflowOptions(options), zonedDateTimeToPlainTime(createTimeZoneOffsetOps, slots);
      }
      return ((bag, options) => {
    ...
    return refineOverflowOptions(options), (s => {
  ...
  function toPlainDateTimeSlots(arg, options) {
    if (options = copyOptions(options), isObjectLike(arg)) {
      ...
      switch (slots.branding) {
       ...
       case ZonedDateTimeBranding:
        return refineOverflowOptions(options), zonedDateTimeToPlainDateTime(createTimeZoneOffsetOps, slots);
      }
      ...
      })(createDateRefineOps(getCalendarSlotFromBag(arg)), arg, options);
    }
    const res = (s => {
      const organized = parseDateTimeLike(requireString(s));
      if (!organized || organized.hasZ) {
  ...
  function toPlainDateSlots(arg, options) {
    if (options = copyOptions(options), isObjectLike(arg)) {
      ...
      switch (slots.branding) {
       ...
       case ZonedDateTimeBranding:
        return refineOverflowOptions(options), zonedDateTimeToPlainDate(createTimeZoneOffsetOps, slots);
      }
  ...
  function createCompoundOpsCreator(adapterFuncs) {
    return calendarSlot => "string" == typeof calendarSlot ? createNativeStandardOps(calendarSlot) : ((calendarProtocol, adapterFuncs) => {
      const keys = Object.keys(adapterFuncs).sort(), boundFuncs = {};
      for (const key of keys) {
  ...
  function refinePublicRelativeTo(relativeTo) {
    if (void 0 !== relativeTo) {
      if (isObjectLike(relativeTo)) {
        ...
        return {
          ...((refineTimeZoneArg, getTimeZoneOps, calendarOps, bag) => {
          ...
          })(refineTimeZoneSlot, createTimeZoneOps, createDateRefineOps(calendar), relativeTo),
          calendar: calendar
        };
      }
      return (s => {
  ...
  function refineCalendarSlot(arg) {
    return isObjectLike(arg) ? (getSlots(arg) || {}).calendar || validateCalendarProtocol(arg) : (arg => resolveCalendarId((s => {
  ...
  function createFormatPrepperForBranding(branding) {
    ...
    return createFormatPrepper(config, memoize(createFormatForPrep));
  }
  ...
  const expectedInteger = (entityName, num) => `Non-integer ${entityName}: ${num}`, expectedPositive = (entityName, num) => `Non-positive ${entityName}: ${num}`, expectedFinite = (entityName, num) => `Non-finite ${entityName}: ${num}`, forbiddenBigIntToNumber = entityName => `Cannot convert bigint to ${entityName}`, invalidBigInt = arg => `Invalid bigint: ${arg}`, forbiddenSymbolToString = "Cannot convert Symbol to string", invalidObject = "Invalid object", numberOutOfRange = (entityName, val, min, max, choices) => choices ? numberOutOfRange(entityName, choices[val], choices[min], choices[max]) : invalidEntity(entityName, val) + `; must be between ${min}-${max}`, invalidEntity = (fieldName, val) => `Invalid ${fieldName}: ${val}`, missingField = fieldName => `Missing ${fieldName}`, forbiddenField = fieldName => `Invalid field ${fieldName}`, duplicateFields = fieldName => `Duplicate field ${fieldName}`, noValidFields = validFields => "No valid fields: " + validFields.join(), invalidChoice = (fieldName, val, choiceMap) => invalidEntity(fieldName, val) + "; must be " + Object.keys(choiceMap).join(), forbiddenEraParts = "Forbidden era/eraYear", mismatchingEraParts = "Mismatching era/eraYear", mismatchingYearAndEra = "Mismatching year/eraYear", invalidEra = era => `Invalid era: ${era}`, missingYear = allowEra => "Missing year" + (allowEra ? "/era/eraYear" : ""), invalidMonthCode = monthCode => `Invalid monthCode: ${monthCode}`, mismatchingMonthAndCode = "Mismatching month/monthCode", missingMonth = "Missing month/monthCode", invalidLeapMonth = "Invalid leap month", invalidProtocolResults = "Invalid protocol results", mismatchingCalendars = "Mismatching Calendars", invalidCalendar = calendarId => `Invalid Calendar: ${calendarId}`, mismatchingTimeZones = "Mismatching TimeZones", forbiddenIcuTimeZone = "Forbidden ICU TimeZone", outOfBoundsOffset = "Out-of-bounds offset", outOfBoundsDstGap = "Out-of-bounds TimeZone gap", invalidOffsetForTimeZone = "Invalid TimeZone offset", ambigOffset = "Ambiguous offset", outOfBoundsDate = "Out-of-bounds date", outOfBoundsDuration = "Out-of-bounds duration", forbiddenDurationSigns = "Cannot mix duration signs", invalidLargeUnits = "Cannot use large units", flippedSmallestLargestUnit = "smallestUnit > largestUnit", failedParse = s => `Cannot parse: ${s}`, invalidSubstring = substring => `Invalid substring: ${substring}`, invalidFormatType = branding => `Cannot format ${branding}`, forbiddenFormatTimeZone = "Cannot specify TimeZone", mapPropNamesToIndex = bindArgs(mapPropNames, ((_propVal, i) => i)), mapPropNamesToConstant = bindArgs(mapPropNames, ((_propVal, _i, constant) => constant)), padNumber2 = bindArgs(padNumber, 2), unitNameMap = {
    ...
    dangi: 13,
    hebrew: -6
  }, RawDateTimeFormat = Intl.DateTimeFormat, isoTimeFieldNamesAsc = [ "isoNanosecond", "isoMicrosecond", "isoMillisecond", "isoSecond", "isoMinute", "isoHour" ], isoDateFieldNamesAsc = [ "isoDay", "isoMonth", "isoYear" ], isoDateTimeFieldNamesAsc = [ ...isoTimeFieldNamesAsc, ...isoDateFieldNamesAsc ], isoDateFieldNamesAlpha = sortStrings(isoDateFieldNamesAsc), isoTimeFieldNamesAlpha = sortStrings(isoTimeFieldNamesAsc), isoDateTimeFieldNamesAlpha = sortStrings(isoDateTimeFieldNamesAsc), isoTimeFieldDefaults = mapPropNamesToConstant(isoTimeFieldNamesAlpha, 0), clearIsoFields = bindArgs(zeroOutProps, isoDateTimeFieldNamesAsc), maxMilli = 1e8 * milliInDay, epochNanoMax = [ 1e8, 0 ], epochNanoMin = [ -1e8, 0 ], isoYearMax = 275760, isoYearMin = -271821, isoEpochOriginYear = 1970, isoEpochFirstLeapYear = 1972, isoMonthsInYear = 12, primaryJapaneseEraMilli = isoArgsToEpochMilli(1868, 9, 8), queryJapaneseEraParts = memoize((isoFields => {
    const epochMilli = isoToEpochMilli(isoFields);
  ...
  const timeRegExpStr = "(\\d{2})(?::?(\\d{2})(?::?(\\d{2})(?:[.,](\\d{1,9}))?)?)?", offsetRegExpStr = "([+−-])" + timeRegExpStr, dateTimeRegExpStr = "(?:(?:([+−-])(\\d{6}))|(\\d{4}))-?(\\d{2})-?(\\d{2})(?:[T ]" + timeRegExpStr + "(Z|" + offsetRegExpStr + ")?)?", yearMonthRegExp = createRegExp("(?:(?:([+−-])(\\d{6}))|(\\d{4}))-?(\\d{2})((?:\\[(!?)([^\\]]*)\\]){0,9})"), monthDayRegExp = createRegExp("(?:--)?(\\d{2})-?(\\d{2})((?:\\[(!?)([^\\]]*)\\]){0,9})"), dateTimeRegExp = createRegExp(dateTimeRegExpStr + "((?:\\[(!?)([^\\]]*)\\]){0,9})"), timeRegExp = createRegExp("T?" + timeRegExpStr + "(?:" + offsetRegExpStr + ")?((?:\\[(!?)([^\\]]*)\\]){0,9})"), offsetRegExp = createRegExp(offsetRegExpStr), annotationRegExp = new RegExp("\\[(!?)([^\\]]*)\\]", "g"), durationRegExp = createRegExp("([+−-])?P(\\d+Y)?(\\d+M)?(\\d+W)?(\\d+D)?(?:T(?:(\\d+)(?:[.,](\\d{1,9}))?H)?(?:(\\d+)(?:[.,](\\d{1,9}))?M)?(?:(\\d+)(?:[.,](\\d{1,9}))?S)?)?"), queryTimeZoneIntlFormat = memoize((id => new RawDateTimeFormat("en-GB", {
    ...
    minute: "numeric",
    second: "numeric"
  }))), icuRegExp = /^(AC|AE|AG|AR|AS|BE|BS|CA|CN|CS|CT|EA|EC|IE|IS|JS|MI|NE|NS|PL|PN|PR|PS|SS|VS)T$/, builtinRefiners = {
  ...
  const timeZoneNameStrs = [ "timeZoneName" ], monthDayFallbacks = {
    ...
    calendarId: slots => getId(slots.calendar)
  }, adapterSimpleOps = mapProps(((refiner, methodName) => function(isoFields) {
  ...
  }), dateRefiners), durationGetters = mapPropNames((propName => slots => slots[propName]), durationFieldNamesAsc.concat("sign")), timeGetters = mapPropNames(((_name, i) => slots => slots[isoTimeFieldNamesAsc[i]]), timeFieldNamesAsc), epochGetters = {
    epochSeconds(slots) {
      return epochNanoToSec(slots.epochNanoseconds);
    },
    epochMilliseconds: getEpochMilli,
    epochMicroseconds(slots) {
      return bigNanoToBigInt(slots.epochNanoseconds, nanoInMicro);
    },
    epochNanoseconds(slots) {
      return bigNanoToBigInt(slots.epochNanoseconds);
    }
  }, removeBranding = bindArgs(excludePropsByName, new Set([ "branding" ])), [PlainMonthDay, createPlainMonthDay, getPlainMonthDaySlots] = createSlotClass(PlainMonthDayBranding, bindArgs(((refineCalendarArg, isoMonth, isoDay, calendar = isoCalendarId, referenceIsoYear = isoEpochFirstLeapYear) => {
    ...
    with(slots, mod, options) {
      return createPlainMonthDay(((getCalendarOps, plainMonthDaySlots, initialFields, modFields, options) => {
      ...
      })(createMonthDayModOps, slots, this, rejectInvalidBag(mod), options));
    },
    equals(slots, otherArg) {
      return !compareIsoDateFields(plainMonthDaySlots0 = slots, plainMonthDaySlots1 = toPlainMonthDaySlots(otherArg)) && isIdLikeEqual(plainMonthDaySlots0.calendar, plainMonthDaySlots1.calendar);
      var plainMonthDaySlots0, plainMonthDaySlots1;
    },
    toPlainDate(slots, bag) {
      return createPlainDate(((getCalendarOps, plainMonthDaySlots, plainMonthDayFields, bag) => ((calendarOps, input, bag) => convertToIso(calendarOps, input, monthCodeDayFieldNames, requireObjectLike(bag), yearFieldNames))(getCalendarOps(plainMonthDaySlots.calendar), plainMonthDayFields, bag))(createDateModOps, slots, this, bag));
    },
    ...
    getPossibleInstantsFor(timeZoneProtocol, getPossibleInstantsFor, isoFields) {
      const epochNanos = [ ...getPossibleInstantsFor.call(timeZoneProtocol, createPlainDateTime(createPlainDateTimeSlots(isoFields, isoCalendarId))) ].map((instant => getInstantSlots(instant).epochNanoseconds)), epochNanoLen = epochNanos.length;
      return epochNanoLen > 1 && (epochNanos.sort(compareBigNanos), validateTimeZoneGap(bigNanoToNumber(diffBigNanos(epochNanos[0], epochNanos[epochNanoLen - 1])))), 
      epochNanos;
    }
  }, simpleTimeZoneAdapters = {
    getOffsetNanosecondsFor: getOffsetNanosecondsForAdapter
  }, [TimeZone, createTimeZone] = createSlotClass("TimeZone", (id => {
    const slotId = (id => resolveTimeZoneId(requireString(id)))(id);
    return {
      branding: "TimeZone",
      id: slotId,
      native: queryNativeTimeZone(slotId)
    };
  }), {
    id: slots => slots.id
  }, {
    getPossibleInstantsFor: ({native: native}, plainDateTimeArg) => native.getPossibleInstantsFor(toPlainDateTimeSlots(plainDateTimeArg)).map((epochNano => createInstant(createInstantSlots(epochNano)))),
    ...
    getOffsetStringFor(_slots, instantArg) {
      ...
      return formatOffsetNano(createAdapterOps(this, simpleTimeZoneAdapters).getOffsetNanosecondsFor(epochNano));
    },
    ...
    getInstantFor(_slots, plainDateTimeArg, options) {
      const isoFields = toPlainDateTimeSlots(plainDateTimeArg), epochDisambig = refineEpochDisambigOptions(options), calendarOps = createAdapterOps(this);
  ...
  }), validateTimeZoneProtocol = createProtocolValidator(Object.keys(timeZoneAdapters)), [PlainYearMonth, createPlainYearMonth, getPlainYearMonthSlots] = createSlotClass(PlainYearMonthBranding, bindArgs(((refineCalendarArg, isoYear, isoMonth, calendar = isoCalendarId, referenceIsoDay = 1) => {
    ...
    getCalendar: createCalendarFromSlots,
    with(slots, mod, options) {
      return createPlainYearMonth(((getCalendarOps, plainYearMonthSlots, initialFields, modFields, options) => {
        const optionsCopy = copyOptions(options);
      ...
      })(createYearMonthModOps, slots, this, rejectInvalidBag(mod), options));
    },
    add: (slots, durationArg, options) => createPlainYearMonth(movePlainYearMonth(createYearMonthMoveOps, 0, slots, toDurationSlots(durationArg), options)),
    subtract: (slots, durationArg, options) => createPlainYearMonth(movePlainYearMonth(createYearMonthMoveOps, 1, slots, toDurationSlots(durationArg), options)),
    until: (slots, otherArg, options) => createDuration(diffPlainYearMonth(createYearMonthDiffOps, 0, slots, toPlainYearMonthSlots(otherArg), options)),
    since: (slots, otherArg, options) => createDuration(diffPlainYearMonth(createYearMonthDiffOps, 1, slots, toPlainYearMonthSlots(otherArg), options)),
    equals(slots, otherArg) {
      return !compareIsoDateFields(plainYearMonthSlots0 = slots, plainYearMonthSlots1 = toPlainYearMonthSlots(otherArg)) && isIdLikeEqual(plainYearMonthSlots0.calendar, plainYearMonthSlots1.calendar);
      var plainYearMonthSlots0, plainYearMonthSlots1;
    },
    toPlainDate(slots, bag) {
      return createPlainDate(((getCalendarOps, plainYearMonthSlots, plainYearMonthFields, bag) => ((calendarOps, input, bag) => convertToIso(calendarOps, input, yearMonthCodeFieldNames, requireObjectLike(bag), dayFieldNames))(getCalendarOps(plainYearMonthSlots.calendar), plainYearMonthFields, bag))(createDateModOps, slots, this, bag));
    },
    toLocaleString(slots, locales, options) {
    ...
    timeZoneId: slots => getId(slots.timeZone),
    hoursInDay: slots => ((getTimeZoneOps, slots) => {
      ...
      return hoursExact;
    })(createTimeZoneOps, slots)
  }, {
    getISOFields: slots => ((getTimeZoneOps, zonedDateTimeSlots) => {
    ...
    })(createTimeZoneOffsetOps, slots),
    getCalendar: createCalendarFromSlots,
    getTimeZone: ({timeZone: timeZone}) => "string" == typeof timeZone ? new TimeZone(timeZone) : timeZone,
    with(slots, mod, options) {
      return createZonedDateTime(((getCalendarOps, getTimeZoneOps, zonedDateTimeSlots, initialFields, modFields, options) => {
      ...
      })(createDateModOps, createTimeZoneOps, slots, this, rejectInvalidBag(mod), options));
    },
    withCalendar: (slots, calendarArg) => createZonedDateTime(slotsWithCalendar(slots, refineCalendarSlot(calendarArg))),
    withTimeZone: (slots, timeZoneArg) => createZonedDateTime(((slots, timeZoneSlot) => ({
      ...slots,
      timeZone: timeZoneSlot
    }))(slots, refineTimeZoneSlot(timeZoneArg))),
    withPlainDate: (slots, plainDateArg) => createZonedDateTime(((getTimeZoneOps, zonedDateTimeSlots, plainDateSlots) => {
    ...
    })(createTimeZoneOps, slots, toPlainDateSlots(plainDateArg))),
    withPlainTime: (slots, plainTimeArg) => createZonedDateTime(((getTimeZoneOps, zonedDateTimeSlots, plainTimeSlots = isoTimeFieldDefaults) => {
      const timeZoneSlot = zonedDateTimeSlots.timeZone, timeZoneOps = getTimeZoneOps(timeZoneSlot), isoFields = {
        ...zonedEpochSlotsToIso(zonedDateTimeSlots, timeZoneOps),
        ...plainTimeSlots
      };
    ...
    })(createTimeZoneOps, slots, optionalToPlainTimeFields(plainTimeArg))),
    add: (slots, durationArg, options) => createZonedDateTime(moveZonedDateTime(createMoveOps, createTimeZoneOps, 0, slots, toDurationSlots(durationArg), options)),
    subtract: (slots, durationArg, options) => createZonedDateTime(moveZonedDateTime(createMoveOps, createTimeZoneOps, 1, slots, toDurationSlots(durationArg), options)),
    until: (slots, otherArg, options) => createDuration(createDurationSlots(diffZonedDateTimes(createDiffOps, createTimeZoneOps, 0, slots, toZonedDateTimeSlots(otherArg), options))),
    since: (slots, otherArg, options) => createDuration(createDurationSlots(diffZonedDateTimes(createDiffOps, createTimeZoneOps, 1, slots, toZonedDateTimeSlots(otherArg), options))),
    ...
    })(createTimeZoneOps, slots, options)),
    startOfDay: slots => createZonedDateTime(((getTimeZoneOps, slots) => {
      const {timeZone: timeZone, calendar: calendar} = slots, epochNano1 = ((computeAlignment, timeZoneOps, slots) => getSingleInstantFor(timeZoneOps, computeAlignment(zonedEpochSlotsToIso(slots, timeZoneOps))))(computeDayFloor, getTimeZoneOps(timeZone), slots);
      return createZonedDateTimeSlots(epochNano1, timeZone, calendar);
    })(createTimeZoneOps, slots)),
    equals(slots, otherArg) {
      return !compareZonedDateTimes(zonedDateTimeSlots0 = slots, zonedDateTimeSlots1 = toZonedDateTimeSlots(otherArg)) && !!isTimeZoneSlotsEqual(zonedDateTimeSlots0.timeZone, zonedDateTimeSlots1.timeZone) && isIdLikeEqual(zonedDateTimeSlots0.calendar, zonedDateTimeSlots1.calendar);
      var zonedDateTimeSlots0, zonedDateTimeSlots1;
    },
    toInstant: slots => createInstant(createInstantSlots(slots.epochNanoseconds)),
    toPlainDateTime: slots => createPlainDateTime(zonedDateTimeToPlainDateTime(createTimeZoneOffsetOps, slots)),
    toPlainDate: slots => createPlainDate(zonedDateTimeToPlainDate(createTimeZoneOffsetOps, slots)),
    toPlainTime: slots => createPlainTime(zonedDateTimeToPlainTime(createTimeZoneOffsetOps, slots)),
    toPlainYearMonth(slots) {
      return createPlainYearMonth(convertToPlainYearMonth(createYearMonthRefineOps(slots.calendar), this));
    },
    toPlainMonthDay(slots) {
      return createPlainMonthDay(convertToPlainMonthDay(createMonthDayRefineOps(slots.calendar), this));
    },
    toLocaleString(slots, locales, options = {}) {
      const [format, epochMilli] = prepZonedDateTimeFormat(locales, options, slots);
      return format.format(epochMilli);
    },
    toString: (slots, options) => formatZonedDateTimeIso(createTimeZoneOffsetOps, slots, options),
    toJSON: slots => formatZonedDateTimeIso(createTimeZoneOffsetOps, slots),
    valueOf: neverValueOf
  }, {
    ...
    toZonedDateTime: (slots, options) => createZonedDateTime(((refineTimeZoneArg, refinePlainDateArg, getTimeZoneOps, slots, options) => {
    ...
    })(refineTimeZoneSlot, toPlainDateSlots, createTimeZoneOps, slots, options)),
    toPlainDateTime(slots, plainDateArg) {
      return createPlainDateTime((plainTimeSlots0 = slots, plainDateSlots1 = toPlainDateSlots(plainDateArg), 
      createPlainDateTimeSlots(checkIsoDateTimeInBounds({
        ...plainTimeSlots0,
        ...plainDateSlots1
      }))));
      var plainTimeSlots0, plainDateSlots1;
    },
    toLocaleString(slots, locales, options) {
    ...
    getCalendar: createCalendarFromSlots,
    with(slots, mod, options) {
      return createPlainDateTime(((getCalendarOps, plainDateTimeSlots, initialFields, modFields, options) => {
        const optionsCopy = copyOptions(options);
      ...
      })(createDateModOps, slots, this, rejectInvalidBag(mod), options));
    },
    withCalendar: (slots, calendarArg) => createPlainDateTime(slotsWithCalendar(slots, refineCalendarSlot(calendarArg))),
    withPlainDate(slots, plainDateArg) {
      return createPlainDateTime((plainDateTimeSlots = slots, plainDateSlots = toPlainDateSlots(plainDateArg), 
      createPlainDateTimeSlots({
        ...plainDateTimeSlots,
        ...plainDateSlots
      }, getPreferredCalendarSlot(plainDateTimeSlots.calendar, plainDateSlots.calendar))));
      var plainDateTimeSlots, plainDateSlots;
    },
    withPlainTime: (slots, plainTimeArg) => createPlainDateTime(((plainDateTimeSlots, plainTimeSlots = isoTimeFieldDefaults) => createPlainDateTimeSlots({
      ...plainDateTimeSlots,
      ...plainTimeSlots
    }))(slots, optionalToPlainTimeFields(plainTimeArg))),
    add: (slots, durationArg, options) => createPlainDateTime(movePlainDateTime(createMoveOps, 0, slots, toDurationSlots(durationArg), options)),
    subtract: (slots, durationArg, options) => createPlainDateTime(movePlainDateTime(createMoveOps, 1, slots, toDurationSlots(durationArg), options)),
    until: (slots, otherArg, options) => createDuration(diffPlainDateTimes(createDiffOps, 0, slots, toPlainDateTimeSlots(otherArg), options)),
    since: (slots, otherArg, options) => createDuration(diffPlainDateTimes(createDiffOps, 1, slots, toPlainDateTimeSlots(otherArg), options)),
    ...
    equals(slots, otherArg) {
      return !compareIsoDateTimeFields(plainDateTimeSlots0 = slots, plainDateTimeSlots1 = toPlainDateTimeSlots(otherArg)) && isIdLikeEqual(plainDateTimeSlots0.calendar, plainDateTimeSlots1.calendar);
      var plainDateTimeSlots0, plainDateTimeSlots1;
    },
    toZonedDateTime: (slots, timeZoneArg, options) => createZonedDateTime(((getTimeZoneOps, plainDateTimeSlots, timeZoneSlot, options) => {
      const epochNano = ((getTimeZoneOps, timeZoneSlot, isoFields, options) => {
      ...
      })(getTimeZoneOps, timeZoneSlot, plainDateTimeSlots, options);
      return createZonedDateTimeSlots(checkEpochNanoInBounds(epochNano), timeZoneSlot, plainDateTimeSlots.calendar);
    })(createTimeZoneOps, slots, refineTimeZoneSlot(timeZoneArg), options)),
    toPlainDate: slots => createPlainDate(createPlainDateSlots(slots)),
    toPlainTime: slots => createPlainTime(createPlainTimeSlots(slots)),
    toPlainYearMonth(slots) {
      return createPlainYearMonth(((getCalendarOps, plainDateTimeSlots, plainDateFields) => {
        const calendarOps = getCalendarOps(plainDateTimeSlots.calendar);
        return createPlainYearMonthSlots({
          ...plainDateTimeSlots,
          ...convertToPlainYearMonth(calendarOps, plainDateFields)
        });
      })(createYearMonthRefineOps, slots, this));
    },
    toPlainMonthDay(slots) {
      return createPlainMonthDay(convertToPlainMonthDay(createMonthDayRefineOps(slots.calendar), this));
    },
    ...
    isoMonth: isoMonth,
    isoDay: isoDay
  }))), refineCalendarArg(calendarArg))), refineCalendarSlot), {
    ...calendarIdGetters,
    ...dateGetters
  }, {
    getISOFields: removeBranding,
    getCalendar: createCalendarFromSlots,
    with(slots, mod, options) {
      return createPlainDate(((getCalendarOps, plainDateSlots, initialFields, modFields, options) => {
        const optionsCopy = copyOptions(options);
        return ((calendarOps, initialFields, modFields, options) => {
          const fields = mergeCalendarFields(calendarOps, initialFields, modFields, dateFieldNamesAlpha);
          return calendarOps.dateFromFields(fields, options);
    ...
    withCalendar: (slots, calendarArg) => createPlainDate(slotsWithCalendar(slots, refineCalendarSlot(calendarArg))),
    add: (slots, durationArg, options) => createPlainDate(movePlainDate(createMoveOps, 0, slots, toDurationSlots(durationArg), options)),
    subtract: (slots, durationArg, options) => createPlainDate(movePlainDate(createMoveOps, 1, slots, toDurationSlots(durationArg), options)),
    until: (slots, otherArg, options) => createDuration(diffPlainDates(createDiffOps, 0, slots, toPlainDateSlots(otherArg), options)),
    since: (slots, otherArg, options) => createDuration(diffPlainDates(createDiffOps, 1, slots, toPlainDateSlots(otherArg), options)),
    equals(slots, otherArg) {
      return !compareIsoDateFields(plainDateSlots0 = slots, plainDateSlots1 = toPlainDateSlots(otherArg)) && isIdLikeEqual(plainDateSlots0.calendar, plainDateSlots1.calendar);
    ...
    toZonedDateTime(slots, options) {
      ...
      return createZonedDateTime(((refineTimeZoneArg, refinePlainTimeArg, getTimeZoneOps, plainDateSlots, options) => {
      ...
      })(refineTimeZoneSlot, toPlainTimeSlots, createTimeZoneOps, slots, optionsObj));
    },
    ...
    add: (slots, otherArg, options) => createDuration(addDurations(refinePublicRelativeTo, createDiffOps, createTimeZoneOps, 0, slots, toDurationSlots(otherArg), options)),
    subtract: (slots, otherArg, options) => createDuration(addDurations(refinePublicRelativeTo, createDiffOps, createTimeZoneOps, 1, slots, toDurationSlots(otherArg), options)),
    ...
    })(refinePublicRelativeTo, createDiffOps, createTimeZoneOps, slots, options)),
    ...
    })(refinePublicRelativeTo, createMoveOps, createTimeZoneOps, toDurationSlots(durationArg0), toDurationSlots(durationArg1), options)
  }), calendarMethods = {
    toString: slots => slots.id,
    toJSON: slots => slots.id,
    ...calendarFieldMethods,
    dateAdd: ({id: id, native: native}, plainDateArg, durationArg, options) => createPlainDate(createPlainDateSlots(native.dateAdd(toPlainDateSlots(plainDateArg), toDurationSlots(durationArg), options), id)),
    dateUntil: ({native: native}, plainDateArg0, plainDateArg1, options) => createDuration(createDurationSlots(native.dateUntil(toPlainDateSlots(plainDateArg0), toPlainDateSlots(plainDateArg1), (options => (options = normalizeOptions(options), 
    refineLargestUnit(options, 9, 6, 1)))(options)))),
    dateFromFields: ({id: id, native: native}, fields, options) => createPlainDate(refinePlainDateBag(native, fields, options, id === isoCalendarId ? [ "year", "day" ] : [])),
    yearMonthFromFields: ({id: id, native: native}, fields, options) => createPlainYearMonth(refinePlainYearMonthBag(native, fields, options, id === isoCalendarId ? yearFieldNames : [])),
    monthDayFromFields: ({id: id, native: native}, fields, options) => createPlainMonthDay(refinePlainMonthDayBag(native, 0, fields, options, id === isoCalendarId ? dayFieldNames : [])),
    fields({native: native}, fieldNames) {
      const allowed = new Set(dateFieldNamesAlpha), fieldNamesArray = [];
      for (const fieldName of fieldNames) {
        if (requireString(fieldName), !allowed.has(fieldName)) {
          throw new RangeError(forbiddenField(fieldName));
        }
    ...
    mergeFields: ({native: native}, fields0, fields1) => native.mergeFields(excludeUndefinedProps(requireNonNullish(fields0)), excludeUndefinedProps(requireNonNullish(fields1)))
  }, [Calendar] = createSlotClass("Calendar", (id => {
    const slotId = (id => resolveCalendarId(requireString(id)))(id);
    return {
      branding: "Calendar",
      id: slotId,
      native: createNativeStandardOps(slotId)
    };
  }), {
    id: slots => slots.id
  }, calendarMethods, {
  ...
  }), validateCalendarProtocol = createProtocolValidator(Object.keys(calendarMethods).slice(4)), [Instant, createInstant, getInstantSlots] = createSlotClass(InstantBranding, (epochNano => createInstantSlots(checkEpochNanoInBounds(bigIntToBigNano(toBigInt(epochNano))))), epochGetters, {
    ...
    toZonedDateTimeISO: (slots, timeZoneArg) => createZonedDateTime(instantToZonedDateTime(slots, refineTimeZoneSlot(timeZoneArg))),
    toLocaleString(slots, locales, options) {
      const [format, epochMilli] = prepInstantFormat(locales, options, slots);
      return format.format(epochMilli);
    },
    toString: (slots, options) => formatInstantIso(refineTimeZoneSlot, createTimeZoneOffsetOps, slots, options),
    toJSON: slots => formatInstantIso(refineTimeZoneSlot, createTimeZoneOffsetOps, slots),
    valueOf: neverValueOf
  }, {
    from: arg => createInstant(toInstantSlots(arg)),
    fromEpochSeconds: epochSec => createInstant((epochSec => createInstantSlots(checkEpochNanoInBounds(numberToBigNano(epochSec, nanoInSec))))(epochSec)),
    fromEpochMilliseconds: epochMilli => createInstant((epochMilli => createInstantSlots(checkEpochNanoInBounds(numberToBigNano(epochMilli, nanoInMilli))))(epochMilli)),
    fromEpochMicroseconds: epochMicro => createInstant((epochMicro => createInstantSlots(checkEpochNanoInBounds(bigIntToBigNano(toBigInt(epochMicro), nanoInMicro))))(epochMicro)),
    ...
    const members = RawDateTimeFormat.prototype, memberDescriptors = Object.getOwnPropertyDescriptors(members), classDescriptors = Object.getOwnPropertyDescriptors(RawDateTimeFormat), DateTimeFormat = function(locales, options = {}) {
      if (!(this instanceof DateTimeFormat)) {
        return new DateTimeFormat(locales, options);
      }
      internalsMap.set(this, ((locales, options = {}) => {
        const rawFormat = new RawDateTimeFormat(locales, options), resolveOptions = rawFormat.resolvedOptions(), resolvedLocale = resolveOptions.locale, copiedOptions = pluckProps(Object.keys(options), resolveOptions), queryFormatPrepperForBranding = memoize(createFormatPrepperForBranding), prepFormat = (...formattables) => {
          let branding;
  ...
  const Now = Object.defineProperties({}, {
    ...
    ...createPropDescriptors({
      ...
      zonedDateTime: (calendar, timeZone = getCurrentTimeZoneId()) => createZonedDateTime(createZonedDateTimeSlots(getCurrentEpochNano(), refineTimeZoneSlot(timeZone), refineCalendarSlot(calendar))),
      zonedDateTimeISO: (timeZone = getCurrentTimeZoneId()) => createZonedDateTime(createZonedDateTimeSlots(getCurrentEpochNano(), refineTimeZoneSlot(timeZone), isoCalendarId)),
      plainDateTime: (calendar, timeZone = getCurrentTimeZoneId()) => createPlainDateTime(createPlainDateTimeSlots(getCurrentIsoDateTime(createTimeZoneOffsetOps(refineTimeZoneSlot(timeZone))), refineCalendarSlot(calendar))),
      plainDateTimeISO: (timeZone = getCurrentTimeZoneId()) => createPlainDateTime(createPlainDateTimeSlots(getCurrentIsoDateTime(createTimeZoneOffsetOps(refineTimeZoneSlot(timeZone))), isoCalendarId)),
      plainDate: (calendar, timeZone = getCurrentTimeZoneId()) => createPlainDate(createPlainDateSlots(getCurrentIsoDateTime(createTimeZoneOffsetOps(refineTimeZoneSlot(timeZone))), refineCalendarSlot(calendar))),
      plainDateISO: (timeZone = getCurrentTimeZoneId()) => createPlainDate(createPlainDateSlots(getCurrentIsoDateTime(createTimeZoneOffsetOps(refineTimeZoneSlot(timeZone))), isoCalendarId)),
      plainTimeISO: (timeZone = getCurrentTimeZoneId()) => createPlainTime(createPlainTimeSlots(getCurrentIsoDateTime(createTimeZoneOffsetOps(refineTimeZoneSlot(timeZone)))))
    })
  ...
  Object.defineProperties(globalThis, createPropDescriptors({
    ...
    toTemporalInstant() {
      return createInstant(createInstantSlots(numberToBigNano(this.valueOf(), nanoInMilli)));
    }
  }));
}();
