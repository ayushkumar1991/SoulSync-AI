...
class GenerativeAgent extends base_js_1.BaseChain {
    ...
    async getEntityFromObservations(observation, runManager) {
        const prompt = prompts_1.PromptTemplate.fromTemplate("What is the observed entity in the following observation? {observation}" +
            "\nEntity=");
    ...
    async getEntityAction(observation, entityName, runManager) {
        const prompt = prompts_1.PromptTemplate.fromTemplate("What is the {entity} doing in the following observation? {observation}" +
            "\nThe {entity} is");
        const result = await this.chain(prompt).call({
            entity: entityName,
            observation,
...
Relevant context:`);
    ...
    async _call(values, runManager) {
        ...
        const prompt = prompts_1.PromptTemplate.fromTemplate(`{agent_summary_description}` +
            `\nIt is {current_time}.` +
